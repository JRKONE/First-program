...
ROOT_DIR = '/home/math13/JR/kaggle/'                                                # 数据集根目录
IMAGE_DIR = os.path.join(ROOT_DIR, "train_v2")                # 训练集图片所在目录
csv_train = 'train_v2train_ship_segmentations_v2.csv'
df = pd.read_csv( os.path.join(ROOT_DIR, csv_train) )  # 读取csv标注文件
...
def save_bad_ann(image_name, mask, segmentation_id):
    ...
    plt.savefig( os.path.join(ROOT_DIR, '../tmp/bad_anns', image_name.split('.')[0] +'_' +str(segmentation_id) +'.png') )
    plt.close()
...
def main():
    ...
    for root, _, files in os.walk(IMAGE_DIR):
        ...
        for image_path in image_paths:
            ...
            print("%d of %d is done."%(image_id,num_of_image_files))    # 显示进度
            image_id = image_id + 1

    with open('home/math13/JR/kaggle/instances_ships_train.json'.format(ROOT_DIR), 'w') as output_json_file:
